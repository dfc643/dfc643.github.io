...
function footerScript() {
    if ( !is_admin() ) {
 	...
 	wp_register_script( 'jquery','//cdn.bootcss.com/jquery/1.8.3/jquery.min.js', false,'1.0');
...
function timeago( $ptime ) {
    ...
    $interval = array (
        6666666666666666666666  =>  ' years ago ('.date('Y-m-d', $ptime).')',
        24 * 60 * 60            =>  'd ago',
        60 * 60                 =>  'h ago',
        60                      =>  'm ago',
        1                       =>  's ago'
    ...
    foreach ($interval as $secs => $str) {
        ...
        if ($d >= 1) {
          ...
          if($etime < 31104000) return date('m-d', $ptime);
          else return date('Y-m-d', $ptime);
        }
    };
}
...
function deel_comment_list($comment, $args, $depth) {
  ...
  echo '<div class="c-main" id="div-comment-'.get_comment_ID().'">';
	...
	if ($comment->comment_approved == '0'){
	  echo '<span class="c-approved">Your comment is reviewed in the queue, please wait!</span><br />';
	}
	//信息
...
err('ERROR: LANGUAGE TYPING SYSTEM HAS BEEN DOWN.');  
} 
...
function hot_posts_list($days=7, $nums=10) { 
    ...
    if(empty($result)) {
        ...
        foreach ($result as $topten) {
            ...
            if ($commentcount != 0) {
              $output .= '<li><p><span class="post-comments"> COMMENT ('.$commentcount.')</span><span class="muted"><a href="javascript:;" data-action="ding" data-id="'.$postid.'" id="Addlike" class="action';
...
//谷歌翻译标题
function google_translate($words) {
    return $words; // 取消使用
    $wd='';
    $ch = curl_init("http://translate.google.cn/translate_a/single?client=gtx&sl=auto&tl=en&hl=zh-CN&dt=t&dt=bd&dj=1&source=icon&q=".urlencode($words));
