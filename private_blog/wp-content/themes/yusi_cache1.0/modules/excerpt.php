...
<?php if( $_thumbnail ){ ?>
	<div class="focus"><a target="_blank" href="<?php the_permalink(); ?>"><img class="thumb" src="<?php echo post_thumbnail_src(); ?>" alt="<?php the_title(); ?>" /></a></div>
	<?php } ?>
		<span class="note"> <?php if (preg_match("/[\x7f-\x80]/", $new_str=deel_strimwidth(strip_tags(apply_filters('the_content', $post->post_content)), 0, 310, ' ...'))) 
		                                                             {echo deel_strimwidth(strip_tags(apply_filters('the_content', $post->post_content)), 0, 140, ' ...');} else {echo $new_str;} ?></span>
<p class="auth-span">
<?php if( !is_author() && !$_author ){ ?>
	...
	<?php if( !$_comment ){ ?><span class="muted"><i class="fa fa-comments-o"></i> <?php 
			if ( comments_open() ) echo '<a target="_blank" href="'.get_comments_link().'">'.get_comments_number('0', '1', '%').' COMMENT</a>'
		?></span><?php } ?>
<?php if( !$_like ){ ?><span class="muted">
<a href="javascript:;" data-action="ding" data-id="<?php the_ID(); ?>" id="Addlike" class="action<?php if(isset($_COOKIE['bigfa_ding_'.$post->ID])) echo ' actived';?>"><i class="fa fa-heart-o"></i><span class="count"><?php if( get_post_meta($post->ID,'bigfa_ding',true) ){ echo get_post_meta($post->ID,'bigfa_ding',true); } else {echo '0';}?></span> LIKE</a></span><?php } ?></p>
</article>
